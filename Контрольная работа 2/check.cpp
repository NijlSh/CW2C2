...
int getBinChoice() 
{
	...
	do {
		...
		{
			std::cout << "Были введены некорректные данные. Повторите ввод: ";
			std::cin.clear();
			std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            continue;
		}
...
int getFigureChoice() 
{
	...
	do {
		...
		{
			std::cout << "Были введены некорректные данные. Повторите ввод: " << std::endl;
			std::cin.clear();
			std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            continue;
		}
...
void checkFileExistance(const std::string file_name, const std::string stream_type) 
{
	...
	{
		std::cout << "Файла с таким названием не существует. Повторите ввод." << std::endl;
		std::cin.clear();
		std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
		throw 1;
	}
